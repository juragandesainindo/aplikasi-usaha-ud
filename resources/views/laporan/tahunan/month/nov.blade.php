        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliNov->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualNov->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliNov->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualNov->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualNov->sum('total_jual'))-($totalBeliNov->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaNov->sum('jumlah_biaya')+$gajiNov->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualNov->sum('total_jual')-$totalBeliNov->sum('total_biaya'))-($biayaNov->sum('jumlah_biaya')+$gajiNov->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualNov->sum('tonase_jual')-$tonaseBeliNov->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualNov->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

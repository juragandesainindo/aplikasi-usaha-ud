        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliApr->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualApr->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliApr->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualApr->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualApr->sum('total_jual'))-($totalBeliApr->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaApr->sum('jumlah_biaya')+$gajiApr->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualApr->sum('total_jual')-$totalBeliApr->sum('total_biaya'))-($biayaApr->sum('jumlah_biaya')+$gajiApr->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualApr->sum('tonase_jual')-$tonaseBeliApr->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualApr->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

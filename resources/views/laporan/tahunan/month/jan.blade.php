    ...
    <div class="table-responsive">
        ...
        <table class="table table-bordered text-center">
            ...
            <tbody>
                ...
                $no=1;
                @endphp
                @foreach($periode->sortBy('bulan') as $key => $item)
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliJan->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualJan->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliJan->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualJan->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualJan->sum('total_jual'))-($totalBeliJan->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaJan->sum('jumlah_biaya')+$gajiJan->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualJan->sum('total_jual')-$totalBeliJan->sum('total_biaya'))-($biayaJan->sum('jumlah_biaya')+$gajiJan->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualJan->sum('tonase_jual')-$tonaseBeliJan->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualJan->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

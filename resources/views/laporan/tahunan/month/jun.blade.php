        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliJun->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualJun->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliJun->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualJun->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualJun->sum('total_jual'))-($totalBeliJun->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaJun->sum('jumlah_biaya')+$gajiJun->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualJun->sum('total_jual')-$totalBeliJun->sum('total_biaya'))-($biayaJun->sum('jumlah_biaya')+$gajiJun->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualJun->sum('tonase_jual')-$tonaseBeliJun->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualJun->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

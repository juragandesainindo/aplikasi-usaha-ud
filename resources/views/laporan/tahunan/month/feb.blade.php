        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliFeb->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualFeb->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliFeb->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualFeb->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualFeb->sum('total_jual'))-($totalBeliFeb->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaFeb->sum('jumlah_biaya')+$gajiFeb->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualFeb->sum('total_jual')-$totalBeliFeb->sum('total_biaya'))-($biayaFeb->sum('jumlah_biaya')+$gajiFeb->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualFeb->sum('tonase_jual')-$tonaseBeliFeb->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualFeb->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

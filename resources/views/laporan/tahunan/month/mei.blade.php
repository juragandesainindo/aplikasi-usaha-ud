        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliMei->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualMei->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliMei->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualMei->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualMei->sum('total_jual'))-($totalBeliMei->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaMei->sum('jumlah_biaya')+$gajiMei->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualMei->sum('total_jual')-$totalBeliMei->sum('total_biaya'))-($biayaMei->sum('jumlah_biaya')+$gajiMei->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualMei->sum('tonase_jual')-$tonaseBeliMei->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualMei->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

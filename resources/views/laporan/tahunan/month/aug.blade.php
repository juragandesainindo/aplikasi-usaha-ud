        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliAug->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualAug->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliAug->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualAug->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualAug->sum('total_jual'))-($totalBeliAug->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaAug->sum('jumlah_biaya')+$gajiAug->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualAug->sum('total_jual')-$totalBeliAug->sum('total_biaya'))-($biayaAug->sum('jumlah_biaya')+$gajiAug->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualAug->sum('tonase_jual')-$tonaseBeliAug->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualAug->sum('tonase_sisa_terjual') }}</th>
                    <th>{{

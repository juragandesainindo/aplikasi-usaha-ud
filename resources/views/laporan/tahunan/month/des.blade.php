        ...
        <table class="table table-bordered text-center">
            ...
            <tfoot style="background: #DAEEF3;">
                <tr>
                    <th></th>
                    <th>Total</th>
                    <th>{{ $tonaseBeliDes->sum('tonase_beli') }}</th>
                    <th>{{ $tonaseJualDes->sum('tonase_jual') }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalBeliDes->sum('total_biaya')) }}</th>
                    <th>Rp.&nbsp;{{ number_format($totalJualDes->sum('total_jual')) }}</th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualDes->sum('total_jual'))-($totalBeliDes->sum('total_biaya')))
                        }}</th>
                    <th>Rp.&nbsp;{{ number_format($biayaDes->sum('jumlah_biaya')+$gajiDes->sum('upah')) }}
                    </th>
                    <th>Rp.&nbsp;{{
                        number_format(($totalJualDes->sum('total_jual')-$totalBeliDes->sum('total_biaya'))-($biayaDes->sum('jumlah_biaya')+$gajiDes->sum('upah')))
                        }}</th>
                    <th>{{ $tonaseJualDes->sum('tonase_jual')-$tonaseBeliDes->sum('tonase_beli') }}</th>
                    <th>{{ $sisaJualDes->sum('tonase_sisa_terjual') }}</th>
                    <th>{{
